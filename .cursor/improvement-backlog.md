...
> **2026-03-03** | Trigger: `slow_pipeline` (25949ms, threshold 25000ms, requestId 6f376be3) | Fixed P1: Pipeline 6f376be3 took 25.9s (breaking_news, 6 cards) — enhance 17.5s + review 6.9s. Root cause: review phase always runs an LLM call when research findings exist, even when all cards are already populated (enhance populated 6/6). Fix: skip LLM review when all cards populated, replace with instant programmatic research enrichment (add sourceUrls, context, fact-check data to matching cards). Expected savings: ~7s per pipeline when enhance populates all cards.
