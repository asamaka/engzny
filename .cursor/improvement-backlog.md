...
> **2026-03-04** | Trigger: `healthcheck` (e698e247 emergency/breaking_news, slow_pipeline 29s) | Fixed P1: LLM-hallucinated imageUrl/photoUrl on cards. Enhance phase has no web access, so every image URL it generates is fabricated — confirmed across 3/4 recent reports (Wikipedia ant on emergency advisory, unsplash URLs, wrong Wikipedia photos). Fix: strip imageUrl/photoUrl from enhance phase callbacks in orchestrator (defense-in-depth), removed prompt instructions asking for image URLs, updated card schema descriptions to say "populated from web research only". Real image URLs still flow through from Sonar research via programmatic enrichment. UI rubric: all 5 criteria PASS (report e698e247, 393px viewport, 9/9 cards, Daisy components).
