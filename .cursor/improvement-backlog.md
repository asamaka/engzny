...
> **2026-03-04** | Trigger: `healthcheck` (db8c99a9 breaking_news report_review) | Fixed P0: Invalid model name `claude-sonnet-4-5-20250514` in orchestrator-v2.js (introduced by `deb67a8 feat(pipeline): restructure to 3-milestone flow`). This model doesn't exist in the Anthropic API, causing a 404 on every enhance phase call. Confirmed via error logs and report 18e871e3 (enhanceDuration=79ms, errors=1, enhanceActions=0). The catch block masked it as non-fatal, so pipelines still completed via the review fallback, but users saw loading skeletons for 15+ seconds instead of progressive card population. Fix: changed to `claude-sonnet-4-20250514` (Claude Sonnet 4). UI rubric: report 18e871e3 — 6/6 cards populated (via review fallback), all 5 criteria PASS structurally (Daisy components, consistent spacing, no clipping, full population, mobile-width OK). Note: enhance phase was non-functional so progressive SSE card updates were absent.
