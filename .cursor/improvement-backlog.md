...
> **2026-03-03** | Trigger: `healthcheck` (1 skipped report, 00c854e0, messaging, 11.2s) | Fixed P2 UI: (1) chat_card renderer had field name mismatches — LLM sends `isOwn`/`timestamp` but renderer checked `isUser`/`time`, causing broken bubble alignment and missing timestamps. Now accepts common variants. (2) Follow-up questions section was always empty — classifier returns `[]` and research follow-up Q&A from Sonar was never surfaced. Now orchestrator merges research follow-up questions into contentAnalysis, and frontend re-renders follow-up section on pipeline complete with enriched Q&A.
