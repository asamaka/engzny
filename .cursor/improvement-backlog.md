...
> **2026-03-03** | Trigger: `report_review` (requestId f38c2ec1, success, 17.4s) | Fixed P1: `onCardUpdated` handler in hub-v2.html did NOT count cards as completed — only `onCardReceived` (for `card` events) and `onCardAdded` (for `card_add` events) incremented `completedCards`. Since the enhance phase sends ALL card populations as `card_update` events, the progress bar showed "1/7" or "2/7" even when all 7 cards were rendered, and client telemetry always reported completedCards=1-2. Fix: `onCardUpdated` now tracks newly populated cards in `populatedCardIds`, increments `completedCards`, triggers first-card-shown transition, updates progress bar text. This also fixes the `entering` animation class for enhance-populated cards.
