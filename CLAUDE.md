...
1. **Freshness floor (deterministic).** Every candidate clip must (a) carry a **known publish date** — so the TV can always render a timestamp on the thumbnail — and (b) be published **at/after the segment's first-reported time** minus a grace window (`INTEL_VIDEO_EVENT_GRACE_H`, default 24h). A clip that predates the event cannot be about it. `searchVideos` now **drops undated clips** (previously an unparseable relative-age slipped a stale clip through with `publishedAt:null`) and accepts a `notBefore` floor; the floor is applied to the broadcaster pool too.
...
| `INTEL_VIDEO_EVENT_GRACE_H` | `24` | Grace (hours) before a segment's first-reported time; clips published earlier than `firstReported − grace` are dropped (recurring-event fix) |
